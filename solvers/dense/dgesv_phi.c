...
int main(int argc, char *argv[]) {
        if (argc != 2) {
          printf("Usgae %s dimension\n", argv[0]);
          exit(1);
        }

	/* Locals */
	MKL_INT n, nrhs = NRHS, lda, ldb = LDB, info;
        n = atoi(argv[1]);
        lda = n;
        int i;
	...
	/* Solve the equations A*X = B */
        ...
        #pragma offload target(mic:0) in(n, nrhs, lda, ldb) in(a:length(size_a) ALLOC) in(ipiv:length(n) ALLOC) in(b:length(n) ALLOC)
        {
	...
	if( info > 0 ) {
        ...
        printf("N = %d  Time = %.3lf ms\n", n, diff/1000);
        // Factorization Flops (2/3)n^3
        // Solution Flops 2n^2
        //printf("n = %d\n", n);
        //double flops = 2 * (0.67 * n * n * n + 2 * n * n);
