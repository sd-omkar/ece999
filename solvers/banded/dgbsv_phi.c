...
int main(int argc, char *argv[]) {
	...
	/* Solve the equations A*X = B */
        ...
        #pragma offload target(mic:0) in(n, kl, ku, nrhs, lda, ldb) in(a:length(size_a) ALLOC) in(ipiv:length(n) ALLOC) in(b:length(n) ALLOC)
        {
        gettimeofday(&begin, NULL);
	info = LAPACKE_dgbsv( LAPACK_ROW_MAJOR, n, kl, ku, nrhs, a, lda, ipiv,
			b, ldb );
